...
def set_image(i,img_float):
    # Convert the NumPy array back to an image
    img_float = (img_float * 1.0).astype(np.uint8)
    img = Image.fromarray(img_float)
    # Save the image
    if i==1:
        img.save('original(I1).jpg')
    if i==2:
        img.save('original(I2).jpg')
    if i==3:
        img.save('encrypted(C1).jpg')
    if i==4:
        img.save('encrypted(C2).jpg')
    if i==5:
...
def calculate_NPCR(image1_path, image2_path):
    ...
    return npcr
#======================================================================================================================================================================
def calculate_UART(image1_path, image2_path):
    # Load input images
    image1 = Image.open(image1_path)
    image2 = Image.open(image2_path)
    ...
    for x in range(width):
        for y in range(height):
            ...
            uart_list.append(abs(total_sum2-total_sum1))
    sum_uart=(sum(uart_list))

    uart=(sum_uart/num_pixels)*100
...
UART=calculate_UART('encrypted(C1).jpg','encrypted(C2).jpg')
print("UART value:",UART)
